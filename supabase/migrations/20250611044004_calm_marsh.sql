/*
  ...
  This migration adds comprehensive project management features including:
  ...
  3. Security
     ...
     - Add policies for project participants to access data
     - Add policies for project creators to manage data
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE tablename = 'project_timelines' AND rowsecurity = true
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'project_resources' AND rowsecurity = true
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'project_updates' AND rowsecurity = true
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'project_discussions' AND rowsecurity = true
  ) THEN
